...
public class Integritymatchagain {
    ...
    public Integritymatchagain(LibProfile libProfile, ApkProfile apkProfile, Set<String> targetSdkClassNameSet) {
...
return Similarity;
    ...
    public static double rawPackageSimilarityfourth(LibPackageProfile libPackageProfile, ApkPackageProfile apkPackageProfile,
            /*as return value*/Map<String, String> classNameMap, /*as return value*/Map<String, Double> classRawSimilarity) {
List<String> ClassList = libPackageProfile.getWeightClassList();
      for (String libClassName : ClassList) {
SimpleClassProfile simpleLibClassProfile = libPackageProfile.classProfileMap.get(libClassName);
...
if (classNameMap.get(libClassName)!=null) {
	//System.out.println("apkClassName:"+classNameMap.get(libClassName)+"---------->"+libClassName);
...
return Similarity;
        ...
        for (LibPackageProfile libPackageProfile : libPackageProfileMap.values()){   
        	...
        	for (ApkPackageProfile apkPackageProfile : apkPackageProfileMapcom.values()) {
        		if (confusedset.contains(apkPackageProfile.packageName)) {
        			Map<String, String> classNameMap = new HashMap<>();
        	        Map<String, Double> classRawSimilarity = new HashMap<>();
        	        double similarity=rawPackageSimilarityfourth(libPackageProfile,apkPackageProfile, classNameMap, classRawSimilarity);
        	        if(similarity>=0){
        	        	if(maxsim<similarity||maxsim==similarity){
              	    	   maxsim=similarity;
              	    	   matchpkgname=apkPackageProfile.packageName;             	    	   
    ...
    public static void start(String apkPath,List<String> libPath0) throws Exception {
       ...
       System.out.println(apkPath);
    	...
    	dexfilepath.addAll(libPath0);//"G:\\libpecker备份\\袁倩婷\\lib340"
        ...
        Map<String,List<String>> rootPackageMap=apkProfile.NotprintrootanalysisMap(apkPath);
    	...
    	for(String libPath:dexfilepath){
            ...
            Integritymatchagain detective = new Integritymatchagain(libProfile, apkProfile, targetSdkClassNameSet);
    	...
    	for (String root:rootPackageMap.keySet()) {
        	...
        	for (String libString:perfectMatchlibList){
        		if (!MatchedLib.keySet().contains(libString)) {
        			...
        			String libPath = null;
        			for (String astring :libPath0) {
						if (astring.contains(libString)) {
							libPath=astring;
						}
					}
        			Lib lib = Lib.loadFromFile(libPath);
