...
public class LibPass {
    ...
    static List<String>notConfusedset=new ArrayList<>();//notConfusedset???????§Ò???????apk????
...
return Similarity;
	...
	public  double secondStep(List<String> confusedset, String libname){
        ...
        System.out.println("-------------?????????,??lib??apk?§Ö???????"+libsim+"-----------------");
    ...
    public static void start(String apkPath,String libPath0) throws Exception {
    	...
    	List<String>verList1=new ArrayList<>();//?????·Ú??
       ...
       System.out.println(apkPath);
        ...
        System.out.println("¦Ä???????????????"+notConfusedset.size()+"??:"+notConfusedset);
    	System.out.println("---------------------------------apk???????------------------------------------");
        //????§»apk??????
  	...
  	//?????????¨®?????candidate??????candidatelist????????????????
    	...
    	for (String root:rootPackageMap.keySet()) {
        	...
        	if(specialmatchlib.size()==1){
        	 ...
        	 System.out.println("??????§á????lib?§µ?"+perfectMatchlibList);
