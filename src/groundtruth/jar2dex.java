...
public class jar2dex {
	...
	private static String batpath = "F:/豌豆荚/batfile/";//G:/LibPecker源码/LibPecker-masteroriginal/LibPecker-master/bin
	...
	public void execute(List<String> jarpath) throws IOException{
		for(String jar:jarpath){
			...
			//jar2dex.clearInfoForFile(batfilepath);
	...
	public static String creatbatFile(String name) throws IOException {
		boolean flag = false;
		String filenameTemp;
		filenameTemp = "G:\\libdetection实验标准采集\\bat\\"+ name + ".bat";
		File filename = new File(filenameTemp);
		if (!filename.exists()) {
			filename.createNewFile();
			flag = true;
