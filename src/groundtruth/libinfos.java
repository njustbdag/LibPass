...
public class libinfos {
	...
	public static void main(String[] args){
		List<String> a=getdexname("G:\\libdetection实验标准采集\\dex库");
		LibInfoService LibInfoService=new LibInfoService();
		for(String a1:a){
			if(LibInfoService.findOneBylibname(a1)==0){
				LibInfoService.addLib(a1);
				System.out.println("插入"+a1+"完成");
			}
		}
		//LibInfo  Lib=LibInfoService.findOneBylibname("edu.testshared1");
	...
	public static List<String> getdexname(String dexpath){
		List<String> a=new ArrayList<>();
		List<String> b=new ArrayList<>();
		aar2dex aa=new aar2dex();
		//a.addAll(aa.traverseFolder1("E:\\GoogletplsCloud"));//jar所在目录
		a.addAll(aa.traverseFolder1(dexpath));//找到该目录下的所有dex文件路径
		for(String a1:a){
